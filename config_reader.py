        ...
        for item in yaml_data:
            for key, value_type in BookingConfig.EXPECTED_STRUCTURE.items():
                ...
                if not isinstance(item[key], value_type):
                    raise TypeError(f'Invalid value type for key "{key}": {item[key]}. Expected {value_type}, but got {type(item[key])}')
...
def main():
    ...
    print(config.get_booking_of_weekday("Tu."))
    print(config.get_booking_of_weekday("Mo."))
